...
If we navigate our browsers to `/events/new` we'll get a friendly little error reminding us that we don't have a template called `events-new.handlebars` yet. So let's put that into our `views` folder.
...
Since Sequelize's documentation is netoriously bad, refer to this alternative documentation I've written called [Sequelize-It](https://ajbraus.github.io/sequelize-it).

Our first step to setup a database is to add PostgreSQL to our enviornment.

## Cloud9

If you have Cloud9 PostgreSQL is already installed and you can use the following steps to start your database.

1. `$ sudo service postgresql start`

## Computer

If you are running your development environment on your computer, you'll have to install postgres. Use the [Postgres.app](https://postgresapp.com/) to simplify this process.
...
This command creates both a model file `event.js`—notice it is sigular, unlike our event controller `events.js` which is plural—and it created a migration file `0984932849328-create-event.js`. That jumble of numbers is actually a date and time signature for the moment it was created. Every migration has one because migrations have to run in order, so they are run in the order of when they were created.

The important parts about are model are just the two attributes and their types—title:STRING, desc:TEXT. A STRING data type in SQL can have 255 bytes, and the TEXT data type can have unlimited text. Later we'll use that `Event.associate` function to associate Rsvps with our event.
...
And our migration is a bit longer and adds a few other attributes to our Event: `id`, `createdAt`, and `updatedAt`. We'll use the `id` to fetch instances of events, and we'll use `createdAt` and `updatedAt` to order them.
...
This runs the migration and adds the `Events` table to your database. Now we can save events to the database.
